<template>
  <div class="list">
    <div class="box">
      <ul>
        <li>
          <span>联系人：</span>
          <input type="text" placeholder="请输入联系人的姓名" @blur="blur1" v-model="name" />
        </li>
        <li style="justify-content:center">
          <div
            class="common"
            :class="{active:itemIndex == k}"
            v-for="(v,k) in sex"
            :key="k"
            @click="change(k)"
          >{{v}}</div>
        </li>
        <li>
          <span>手机号：</span>
          <input type="text" placeholder="请输入联系人的电话" @blur="blur2" v-model="phone" />
        </li>
        <li @click="showAdd">
          <span>地址栏：</span>
          <input
            type="text"
            placeholder="请选择您的地理位置"
            disabled="disabled"
            id="cityName"
            style="background: none;"
            v-model="addr"
          />
          <img src="/static/img/dingwei.png" alt />
        </li>
        <li>
          <span>门牌号：</span>
          <input type="text" placeholder="请输入详细地址 如门牌号" @blur="blur3" v-model="add" />
        </li>
        <li>
          <van-checkbox value="checked" @click="checkbox">设为默认</van-checkbox>
        </li>
      </ul>
      <button @click="addbtn">保存</button>
    </div>
  </div>
</template>

<script>
import Request from "@/common/js/request";
// import getCurrentCityName from "@/common/js/location";
export default {
  data() {
    return {
      text: "编辑收货地址",
      radio: "1",
      checked: true,
      mo: 0,
      sex: ["女士", "先生"],
      itemIndex: 0,
      show: false,
      addr: "",
      addID: "",
      name: "",
      phone: "",
      add: "",
      province: "",
      city: "",
      town: ""
    };
  },
  onLoad(options) {
    this.cid = JSON.parse(wx.getStorageSync("user")).cid;
    let adduser = wx.getStorageSync("adduser");

    if (adduser != null) {
      this.name = JSON.parse(adduser).receiverName;
      this.phone = JSON.parse(adduser).phone;
      this.addID = JSON.parse(adduser).addID;
      this.addr = JSON.parse(adduser).address.address;
      this.lon = JSON.parse(adduser).address.lng;
      this.lat = JSON.parse(adduser).address.lat;
      this.province = JSON.parse(adduser).address.prov;
      this.city = JSON.parse(adduser).address.city;
      this.town = JSON.parse(adduser).address.town;
      this.add = JSON.parse(adduser).address.addname;
    }
  },
  components: {
    MainHeader
  },
  mounted() {
  },
  methods: {
    adds(id) {
      let add = {
        cmd: "detailAddress",
        id: id
      };
      Request.postRequest(add)
        .then(res => {
          console.log(res.data);
          if (res.data.result == "0") {
            this.addr = res.data.province + res.data.city;
            this.addID = res.data.id; //地址id
            this.lat = res.data.lat;
            this.lon = res.data.lon;
            this.name = res.data.receiverName; //收货人姓名
            this.phone = res.data.mobile; //手机号
            this.itemIndex = res.data.call; //先生/女士
            this.province = res.data.province; //省
            this.city = res.data.city; //市
            this.town = res.data.town; //区
            this.add = res.data.addr; //详细地址
            if (res.data.default == 0) {
              //是否是默认地址
              this.checked = false;
            } else {
              this.checked = true;
            }
          } else if (res.data.result == "2") {
            this.$router.push("/fenghao");
          }
        })
        .catch(res => {});
    },
    change(k) {
      this.itemIndex = k;
      console.log(k);
    },
    checkbox() {
      if (this.checked==true) {
        this.mo = 1;
      } else {
        this.mo = 0;
      }
    },

    showAdd() {
      this.$router.push("/pages/address/position");
      let adduser = {};
      adduser.name = this.name;
      adduser.phone = this.phone;
      adduser.addID = this.addID;
      wx.setStorageSync("adduser", JSON.stringify(adduser));
    },
    cancel() {
      this.show = false;
    },
    blur1(e) {
      this.name = e.target.value.replace(/(^\s*)/g, "");
    },
    blur2(e) {
      this.phone = e.target.value.replace(/(^\s*)/g, "");
    },
    blur3(e) {
      this.add = e.target.value.replace(/(^\s*)/g, "");
    },
    addbtn() {
      console.log("编辑地址");
      console.log(this.mo);
      if (
        this.name == "" ||
        this.phone == "" ||
        this.province == "" ||
        this.city == "" ||
        this.town == "" ||
        this.add == ""
      ) {
        Toast("信息不能为空");
      } else {
        let add = {
          cmd: "updateAddress",
          id: this.addID,
          cid: this.cid,
          receiverName: this.name,
          call: this.itemIndex,
          mobile: this.phone,
          province: this.province,
          city: this.city,
          town: this.town,
          addr: this.add,
          lon: this.lon,
          lat: this.lat,
          isDefault: this.mo
        };
        console.log(add);
        Request.postRequest(add)
          .then(res => {
            console.log(res.data);
            if (res.data.result == "0") {
              Toast("编辑地址成功");
              localStorage.removeItem("adduser");
              this.$router.push("/address");
            } else if (res.data.result == "2") {
              this.$router.push("/fenghao");
            }
          })
          .catch(res => {});
      }
    }
  }
};
</script>

<style scoped lang="stylus" rel="stylesheet/stylus">
.active {
  background: #72D141;
  color: #fff !important;
  border: none !important;
}

.list {
  width: 100%;
  height: 100%;

  .box {
    width: 100%;
    display: flex;
    flex-direction: column;
    box-sizing: border-box;

    ul {
      width: 100%;
      display: flex;
      flex-direction: column;
      border-top: 1px solid #eee;

      li {
        width: 100%;
        display: flex;
        align-items: center;
        font-size: 14px;
        color: #333;
        padding: 0.4rem 0.6rem;
        box-sizing: border-box;
        border-bottom: 1px solid #eee;

        span {
          display: inline-block;
          width: 2rem;
        }

        img {
          width: 0.35rem;
          height: 0.5rem;
          margin-left: 1rem;
        }

        .common {
          height: 0.7rem;
          line-height: 0.7rem;
          padding: 0 0.3rem;
          border: 1px solid #999;
          font-size: 14px;
          color: #999;
          border-radius: 4px;
          margin-right: 0.7rem;
        }
      }
    }

    button {
      width: 100%;
      position: fixed;
      left: 0;
      bottom: 0;
      border: none;
      background: #72d141;
      color: #fff;
      height: 1.2rem;
    }
  }
}
</style>
